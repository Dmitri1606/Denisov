    ...
    try:
        if message.document.mime_type == 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet':
            ...
            try:
                ...
                table_string = df_excel.to_string()  # Преобразуем всю таблицу в строку

                # Разбиваем строку на части, чтобы поместиться в ограничение Telegram
                chunk_size = 5000
                for i in range(0, len(table_string), chunk_size):
                    chunk = table_string[i:i + chunk_size]
